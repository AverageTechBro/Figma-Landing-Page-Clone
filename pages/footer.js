export default function Footer() {
  return (
    <div className="footer-container">
      <div className="top-section">
        <div className="top-footer-slide">
          <h3>Laptop</h3>
          <ul>
            <li>Acer</li>
            <li>Mac</li>
            <li>HP</li>
            <li>Dell</li>
            <li>Lenovo</li>
          </ul>
        </div>
        <div className="top-footer-slide">
          <h3>Projector</h3>
          <ul>
            <li>Infocus Genesis</li>
            <li>Infocus in IN114AA</li>
            <li>infocus in IN134 400</li>
            <li>Infocus in 134HD</li>
          </ul>
        </div>
        <div className="top-footer-slide">
          <h3>Projector</h3>
          <ul>
            <li>Infocus Genesis</li>
            <li>Infocus in IN114AA</li>
            <li>infocus in IN134 400</li>
            <li>Infocus in 134HD</li>
          </ul>
        </div>
        <div className="top-footer-slide">
          <h3>INFORMATION</h3>
          <ul>
            <li>Terms & conditions</li>
            <li>FAQs</li>
            <li>Payment Methods</li>
            <li>Pricacy Policy</li>
            <li>Shipping and Handling</li>
            <li>Customer Service</li>
          </ul>
        </div>
        <div className="top-footer-slide">
          <h3>CONTACT</h3>
          <ul>
            <li>
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075980940007981086/Frame.png"
                alt=""
              />
              Putalisadak, Opposite Machhapuchhre Bank
            </li>
            <li>
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075980280625627206/Vector15.png"
                alt=""
              />
              01-5342590/91/9823838112
            </li>
            <li>
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075980280441086012/Vector16.png"
                alt=""
              />
              info@Btplanet.com.Np/Btplanetstore12@gmail.com
            </li>
            <li>
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075980280071991416/Vector17.png"
                alt=""
              />
              http://btplanet.com.np
            </li>
          </ul>
        </div>
      </div>
      <div className="bottom-section">
        <div className="download-section">
          <p>Shop Smarter with our apps</p>
          <div className="image-app">
            <img
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978412138704926/image_185.png"
              alt=""
            />
            <img
              className="apple"
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978411824123954/image_186.png"
              alt=""
            />
          </div>
        </div>
        <div className="payment-option-section">
          <p>We Accept</p>
          <div className="payment-image-section">
            <img
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978411299848262/image_187.png"
              alt=""
            />
            <img
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978410989457498/image_188.png"
              alt=""
            />
            <img
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978410586816555/image_189.png"
              alt=""
            />
            <img
              src="https://media.discordapp.net/attachments/1075680327718141992/1075978410293202984/image_191.png"
              alt=""
            />
          </div>
        </div>
        <div className="social-media-section">
          <p>Follow US</p>
          <div className="social-image-section">
            <div className="google">
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075987022315003914/Group_3331.png"
                alt=""
              />
              <p>Google</p>
            </div>
            <div className="twitter">
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075987212224692304/Group_3332.png"
                alt=""
              />
              <p>Twitter</p>
            </div>
            <div className="facebook">
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075987212056928256/Group_3333.png"
                alt=""
              />
              <p>Facebook</p>
            </div>
            <div className="apple">
              <img
                src="https://media.discordapp.net/attachments/1075680327718141992/1075987211876577410/Group_3334.png"
                alt=""
              />
              <p>Apple</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
